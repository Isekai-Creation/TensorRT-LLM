...
TRT=/kaggle/input/tensorrt-10-14-1-48/TensorRT-10.14.1.48
...
mkdir -p "$TRT_SDK/lib"
ln -sfn "$TRT/include" "$TRT_SDK/include"
for base in libnvinfer libnvinfer_plugin libnvonnxparser; do
  ln -sf "$TRT/lib/${base}.so.10.14.1" "$TRT_SDK/lib/${base}.so.10"
  ln -sf "$TRT/lib/${base}.so.10.14.1" "$TRT_SDK/lib/${base}.so"
done

export LD_LIBRARY_PATH="$TRT_SDK/lib:$DRV:/usr/local/nvidia/lib64:$TRT/lib:${LD_LIBRARY_PATH:-}"
export LIBRARY_PATH="$TRT_SDK/lib:$DRV:/usr/local/nvidia/lib64:/usr/local/cuda/lib64/stubs:${LIBRARY_PATH:-}"
